...
%\theoremstyle{remark}
\newdef{property}{Property}

\usepackage{hyperref}
\hypersetup{
pdfauthor = {Nikolaos Pothitos and Panagiotis Stamatopoulos},
pdftitle = {Piece of Pie Search: Confidently Exploiting Heuristics},
pdfsubject = {Computer Science, Artificial Intelligence},
pdfkeywords = {randomness, stochastic methods, discrepancy, constructive search, confidence, CSP},
...
<concept>
<concept_id>10002950.10003714.10003716.10011136.10011797</concept_id>
<concept_desc>Mathematics of computing~Optimization with randomized search heuristics</concept_desc>
...
\ccsdesc[100]{Computing methodologies~Heuristic function construction}
\ccsdesc[300]{Mathematics of computing~Optimization with randomized search heuristics}
\ccsdesc[300]{Theory of computation~Theory of randomized search heuristics}

\CopyrightYear{2016} 
\setcopyright{acmcopyright}
\conferenceinfo{SETN '16,}{May 18-20, 2016, Thessaloniki, Greece}
\isbn{978-1-4503-3734-2/16/05}\acmPrice{\$15.00}
\doi{http://dx.doi.org/10.1145/2903220.2903242}

\title{Piece of Pie Search: Confidently Exploiting Heuristics}
\numberofauthors{2}
\author{
\alignauthor
Nikolaos Pothitos\\
...
\sharedaffiliation
\affaddr{Department of Informatics and Telecommunications}\\
\affaddr{National and Kapodistrian University of Athens}\\
\affaddr{Panepistimiopolis, 157\,84 Athens, Greece}
}
...
\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{
\begin{tikzpicture}
...
\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{\input{figures/heuristics}}
\caption{Heuristic estimations $h_i$ for each value $v_i$\label{heuristics}}
\end{figure}

\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{\input{figures/uniform}}
\caption{The probability is spread uniformly\label{uniform}}
\end{figure}

\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{\input{figures/systematic}}
\caption{Systematic search favours the highest $h_i$\label{systematic}}
\end{figure}

\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{\input{figures/conf}}
...
\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{\input{figures/costsA}}
\caption{Timetabling solutions costs vs.\ $\mathsf{conf}$\label{costsA}}
\end{figure}

\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{\input{figures/costsB}}
...
\begin{figure}
\centering
\resizebox{\columnwidth}{\totalheight}{\input{figures/PoPS/ITC1}}
...
We also thank Foivos Theocharis who initially built the search methods library \textsc{Amorgos}, which is available together with \textsc{Naxos} \cite{naxos-solver}.
...
P.~Barahona, L.~Krippahl, and O.~Perriquet.
\newblock Bioinformatics: A challenge to constraint programming.
\newblock In P.~Van~Hentenryck and M.~Milano, editors, {\em Hybrid
  Optimization}, volume~45, pages 463--487. Springer, New York, 2011.

\bibitem{dbs}
R.~Bart{\'a}k.
\newblock Incomplete depth-first search techniques: A short survey.
\newblock In {\em CPDC 2004: 6$^\mathit{th}$ Workshop on Constraint Programming
  for Decision and Control}, pages 7--14, 2004.

\bibitem{lan}
R.~Bart{\'a}k and H.~Rudov{\'a}.
\newblock Limited assignments: A new cutoff strategy for incomplete depth-first
  search.
\newblock In H.~Haddad, L.~M. Liebrock, A.~Omicini, and R.~L. Wainwright,
  editors, {\em SAC 2005: Symposium on Applied Computing, Santa Fe, New
  Mexico}, pages 388--392, New York, 2005. ACM.

\bibitem{bresina-hbss}
J.~L. Bresina.
\newblock Heuristic-biased stochastic sampling.
\newblock In W.~J. Clancey and D.~S. Weld, editors, {\em AAAI 1996:
  13$^\mathit{th}$ National Conference on Artificial Intelligence, Portland,
  Oregon}, volume~1, pages 271--278, Menlo Park, 1996. AAAI Press.

\bibitem{radio-link}
B.~Cabon, S.~de~Givry, L.~Lobjois, T.~Schiex, and J.~P. Warners.
\newblock Radio link frequency assignment.
\newblock {\em Constraints}, 4(1):79--89, 1999.

\bibitem{cicirello-vbss}
V.~A. Cicirello and S.~F. Smith.
\newblock Enhancing stochastic search performance by value-biased randomization
  of heuristics.
\newblock {\em Journal of Heuristics}, 11(1):5--34, 2005.

\bibitem{eclipse}
{ECL$^i$PS$^e$} constraint logic progr.
\newblock \url{http://eclipseclp.org}, 2012.

\bibitem{p-np}
L.~Fortnow.
\newblock The status of the {P} versus {NP} problem.
\newblock {\em Communications of the ACM}, 52(9):78--86, 2009.

\bibitem{grand-challenges}
E.~C. Freuder and B.~O'Sullivan.
\newblock Grand challenges for constraint programming.
\newblock {\em Constraints}, 19(2):150--162, 2014.

\bibitem{CSPLib}
I.~P. Gent and T.~Walsh.
\newblock \textsc{CSPlib}: A benchmark library for constraints.
\newblock In J.~Jaffar, editor, {\em CP 1999: 5$^\mathit{th}$ International
  Conference on Principles and Practice of Constraint Programming, Alexandria,
  Virginia}, volume 1713 of {\em LNCS}, pages 480--481, Heidelberg, 1999.
  Springer.
\newblock \url{http://CSPLib.org}.

\bibitem{ibroad}
M.~L. Ginsberg and W.~D. Harvey.
\newblock Iterative broadening.
\newblock {\em Artificial Intelligence}, 55(2-3):367--383, 1992.

\bibitem{equivalence}
C.~P. Gomes, B.~Selman, N.~Crato, and H.~Kautz.
\newblock Heavy-tailed phenomena in satisfiability and constraint satisfaction
  problems.
\newblock {\em Journal of Automated Reasoning}, 24(1-2):67--100, 2000.\balancecolumns

\bibitem{ilog}
\textsc{Ilog Solver}.
\newblock \url{http://ilog.com/products/cp}, 2012.

\bibitem{heuristics}
B.~Jafari and M.~Mouhoub.
\newblock Heuristic techniques for variable and value ordering in {CSPs}.
\newblock In {\em GECCO 2011: Proceedings of the 13$^\mathit{th}$ Annual
  Conference on Genetic and Evolutionary Computation, Dublin}, pages 457--464,
  New York, 2011. ACM.

\bibitem{itc-agenda}
B.~McCollum, A.~Schaerf, B.~Paechter, P.~McMullan, R.~Lewis, A.~J. Parkes,
  L.~Di~Gaspero, R.~Qu, and E.~K. Burke.
\newblock Setting the research agenda in automated timetabling: The second
  international timetabling competition.
\newblock {\em INFORMS Journal on Computing}, 22(1):120--130, 2010.

\bibitem{pinedo-scheduling}
M.~L. Pinedo.
\newblock {\em Scheduling: Theory, Algorithms, and Systems}.
\newblock Springer, New York, 4$^\mathrm{th}$ edition, 2012.

\bibitem{naxos-solver}
N.~Pothitos.
\newblock \textsc{Naxos}.
\newblock \url{http://di.uoa.gr/~pothitos/naxos}, 2013.

\bibitem{pothitos-ictai2012}
N.~Pothitos, P.~Stamatopoulos, and K.~Zervoudakis.
\newblock Course scheduling in an adjustable constraint propagation schema.
\newblock In {\em ICTAI 2012: 24$^\mathit{th}$ IEEE International Conference on
  Tools with Artificial Intelligence}, volume~1, pages 335--343. IEEE, 2012.

\bibitem{lds}
P.~Prosser and C.~Unsworth.
\newblock Limited discrepancy search revisited.
\newblock {\em J. Experim. Algor.}, 16:1.6:1--1.6:18, 2011.

\bibitem{roulette}
D.~Sharma, V.~Singh, and C.~Sharma.
\newblock {GA}-based scheduling of {FMS} using roulette wheel selection
  process.
\newblock In {\em SocProS 2011: International Conference on Soft Computing for
  Problem Solving}, volume 131, pages 931--940. Springer, 2012.

\bibitem{CSP}
E.~Tsang.
\newblock {\em Foundations of Constraint Satisfaction}.
\newblock Books on Demand, Norderstedt, 2014.

\bibitem{dds}
T.~Walsh.
\newblock Depth-bounded discrepancy search.
\newblock In M.~E. Pollack, editor, {\em IJCAI 1997: 15$^\mathit{th}$
  International Joint Conference on Artificial Intelligence, Nagoya, Japan},
  volume~2, pages 1388--1393, San Francisco, 1997. Morgan Kaufmann.

\bibitem{learn}
Y.~Xu, D.~Stern, and H.~Samulowitz.
\newblock Learning adaptation to solve constraint problems.
\newblock In {\em LION 3: 3$^\mathit{rd}$ International Conference on Learning
