...
\begin{figure}
  ...
  \begin{tikzpicture}
    [level distance = 3em, sibling distance = 12em,
     recursion/.style = {->, dotted, bend right}]
    \node (Label) {\textsc{LabelPopsSample}($\mathscr{X}$)}
      child {node {\textsf{AND}}
        ...
        child {node (LabelChild) {\textsc{LabelPopsSample}($\mathscr{X} \! - \! \{X\}$)}}
      };
    \path
...
\begin{itemize}
  ...
  \item \texttt{Instantiate3}($X$,
                              $\mathit{CoveredPiece}$) :=
          \textsf{failure},
          if $\mathit{CoveredPiece} > \mathit{Breadth}$,
  \item \texttt{Instantiate3}($X$,
                              $\mathit{CoveredPiece}$) :=
          \textsf{OR}($X \! \gets \! v$, \\
            \hspace*{\stretch{1}}
            \textsf{AND}($D_X \!\! \gets \!\!
              ...
              \texttt{Instantiate3}($X$,
                $\mathit{CoveredPiece} + 1$))), otherwise.
\end{itemize}
