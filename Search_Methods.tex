...
\begin{abstract}
  ...
  from a portfolio of search methods already in place.
...
stages. We present a declarative way to describe ad hoc
...
\subsection{Goals' Satisfaction Algorithm}
...
\begin{figure}
  ...
  \begin{algorithmic}
      ...
      \While {time limit has not been reached}
        ...
        \Else
          ...
          \State $\mathrm{stackOr.top.delayedGoal} \gets
                 ...
                 the \\ stackOr frame that ``delayedGoal''
                 points to \textbf{or} (if there is not such
                 a goal) get the next delayed goal of that
                 stackOr frame
        \EndIf
        \If {$CurrGoal$ is an AND-goal}
        ...
        \Else
          ...
          \If {$\Call{TwoConsistent}{}() = \mathrm{false}$}
          ...
          \ElsIf {stackOr.top.stackAnd is empty \textbf{and} \\
                  $\mathrm{stackOr.top.delayedGoal} = NIL$}
            \State \textbf{return} true \Comment {Success}
          \EndIf
        \EndIf
      \EndWhile
      \State \textbf{return} false
    \EndFunction
  \end{algorithmic}
  \caption{Core Search Algorithm\label{search}}
\end{figure}
...
\begin{lemma}
  ...
  call equals $O(d^n \cdot \mathsf{PieceToCover}^n)$.
\end{lemma}
\begin{proof}
  An initial
  \textsc{PopsSample}$(\mathsf{PieceToCover},\mathsf{conf})$
...
\begin{table}
  \tbl{Solution costs for 14 ITC instances\label{PopsItc}}
  {\begin{tabular}{ccccc}
    \toprule
