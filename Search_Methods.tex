...
graph for the \emph{Iterative Broadening} search method. The
goals' structure is similar to DFS. However, one basic
difference is that there is one more level, namely
\texttt{Broadening}, above the ordinary DFS goals.
\begin{itemize}
...
$d$, then the \texttt{Broadening} goal fails.
...
\newcommand{\Solve}{\textbf{\normalsize S\small OLVE}}

\subsection{\Solve: An algorithm that satisfies search
            methods' goals}

In the previous sections we described a high level way to
define search methods. This framework consists of goals that
support recursion (goals that return goals or success) and
meta-goals used to combine other goals in a conjunctive
(\textsf{AND}) or disjunctive (\textsf{OR}) manner.

This is a not only a theoretical model; it has been
implemented in a C++ Constraint Programming \textsc{Naxos
Solver}. DFS and Iterative Broadening have been declared in
\textsc{Naxos} without loosing much of the above
...
CSP; we need a procedure that satisfies these goals.
  ...
  \begin{algorithmic}
    \Function{Solve}{}
      \While {time limit has not been reached}
        \If {stackOr.top.stackAnd is not empty}
          \State $\mathit{Goal} \gets
                  \mathrm{stackOr.top.stackAnd.pop()}$
        \Else
          \State $\mathit{Goal} \gets
                  \mathrm{stackOr.top.pending}$
          \State $\mathrm{stackOr.top.pending} \gets
                 \textrm{get}$ the next goal of stackAnd of
                 the \\ \hspace{8em} stackOr frame that
                 ``pending'' points to \textbf{or} (if there
                 is not \\ \hspace{8em} such a goal) get the
                 next pending goal of that stackOr frame
        \EndIf
        ...
        \ElsIf {$\mathit{Goal}$ is an OR-goal}
          \State stackOr.push(2\textsuperscript{nd} sub-goal
                 of $\mathit{Goal}$)
          \State Initialize stackOr.top.pending
          \State stackOr.top.stackAnd.push(1\textsuperscript{st}
                 sub-goal of $\mathit{Goal}$)
        \Else
          \State $\mathit{NextGoal} \gets \textrm{result
                  of }\mathit{Goal}\textrm{'s execution}$
          \If {$\Call{PropagateConstraints}{}() = \mathrm{false}$}
            \If {$\Call{BackTrack}{}() = \mathrm{false}$}
              \State \Return false
            \EndIf
          \EndIf
          \If {$\mathit{NextGoal} \neq \textrm{null}$}
            \State stackOr.top.stackAnd.push($\mathit{NextGoal}$)
          \ElsIf {stackOr.top.stackAnd is empty \textbf{and}
                  \\ \hspace{7.5em}
                  $\mathrm{stackOr.top.pending} =
                  \textrm{null}$}
...
\begin{figure}
  ...
  \caption{\textsc{Solve} with DFS example: Part A: $X_1$
           instantiation\label{abcd}}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{figures/stacks/efgh}
  \caption{\textsc{Solve} with DFS example: Part B: $X_2$
           instantiation attempt\label{efgh}}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{figures/stacks/ijkl}
  \caption{\textsc{Solve} with DFS example: Part C: $X_2$
           successful instantiation\label{ijkl}}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{figures/stacks/mnop}
  \caption{\textsc{Solve} with DFS example: Part D: $X_3$
           instantiation attempt\label{mnop}}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{figures/stacks/qrs}
  \caption{\textsc{Solve} with DFS example: Part E: $X_3$
           successful instantiation\label{qrs}}
\end{figure}

\begin{figure}
  \includegraphics[width=\textwidth]{figures/stacks/tuv}
  \caption{\textsc{Solve} with DFS example: Part F: $X_4$
...
\begin{figure}
  ...
  \begin{tikzpicture}
    [level distance = 3em, sibling distance = 12em,
     recursion/.style = {->, dotted, bend right}]
    \node (Label) {\textsc{PopsSample}($\mathscr{X}$)}
      child {node {\textsf{AND}}
        child {node (Instantiate) {\texttt{Instantiate3}($X$)}
          child {node {\textsf{OR}}
            child {node {$X \! \gets \! v$}}
            child {node {\textsf{AND}}
              child {node {$D_X \!\! \gets \!\! D_X \!\! - \!\! \{v\}$}}
              child {node (InstantiateChild) {\texttt{Instantiate3}($X$)}}
            }
          }
        }
        child {node (LabelChild) {\textsc{PopsSample}($\mathscr{X} \! - \! \{X\}$)}}
      };
    \path
