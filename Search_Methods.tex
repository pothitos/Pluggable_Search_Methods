...
Fig.~\ref{abcd}. Each subfigure from (a) to (v) displays a
snapshot of the ``stack of stacks'' while trying to satisfy
the \texttt{Label}($\mathscr{X}$) goal for the
Thessaly-coloring problem. Here we focus on the data
structure itself.
\begin{itemize}
...
The algorithm is wrapped into a ``while'' loop. In each
iteration a unique goal is examined.
...
returned value to the $\mathit{NextGoal}$ variable. A
